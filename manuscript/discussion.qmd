...
In this study, we developed and evaluated an FCS MI algorithm based on the MARS method and compared its performance against other established MI methods through a simulation study. Our findings suggest that the MARS-based MI algorithm performed well, particularly in terms of bias, and is the only method that consistently achieves the targeted coverage level. This is a noteworthy result, as coverage accuracy is a critical criterion for evaluating the validity of MI methods, ensuring that confidence intervals are appropriately calibrated to reflect true parameter uncertainty [@Rubin1987].


<!-- One aspect that likely contributed to the strong coverage performance of the MARS-based MI algorithm is its relatively higher standard errors compared to other MI methods. -->
One factor that likely contributed to the strong coverage performance of the MARS-based MI algorithm is its relatively larger standard errors compared with other MI methods.
...
<!-- New paragraph -->
More generally, the proposed MARS-based MI algorithm shares some of the usual limitations of flexible, data-adaptive imputation methods. In particular, its performance may depend on the structure of the data and on tuning choices, such as the allowed degree of interactions or model complexity. Thus, while the flexibility of MARS is attractive for capturing nonlinear relationships, it may in some settings come at the cost of efficiency, as reflected in the comparatively larger standard errors observed here. At the same time, because MARS uses spline-based basis expansions rather than recursive partitioning, it may be particularly advantageous for continuous variables, where smooth functional forms are plausible.
...
Furthermore, our study does not address situations where the missingness mechanism is not MAR, such as when missingness depends on the outcome variable itself.
This presents a significant area for further exploration, as MI methods often assume MAR, and deviations from this assumption can lead to biased results.
Investigating the application and performance of the MARS-based MI method under MNAR conditions could provide valuable insights into its broader applicability.
